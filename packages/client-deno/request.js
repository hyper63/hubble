import { crocks, create, ms } from './deps.js'
...
export default function(client='noname', secret='nosecret') {
  ...
  function generateHeaders(client, secret) {
    ...
    const sign = Async.fromPromise(create)
    const doVerify = Async.fromPromise(verify)

    return Async.of({[CONTENT_TYPE]: APPLICATION_JSON})
      .map(headers => {
        if (!token) {
          token = sign({ alg: 'HS256', type: 'JWT' }, 
            { sub: client, exp: ms('1h') }, 
            secret
          )
        }
      ...
      .chain(headers => Async(function (reject, resolve) {
        ...
        verify(token, secret, (err) => {
          if (err && err.name === 'TokenExpiredError') {
            token = create({sub: client}, secret, {expiresIn: '1h'})
